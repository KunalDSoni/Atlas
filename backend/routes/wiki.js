...
router.get('/wiki/spaces/:id', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const space = queryOne(db, 'SELECT s.*, u.name as owner_name FROM wiki_spaces s LEFT JOIN users u ON s.owner_id = u.id WHERE s.id = ?', [req.params.id]);
...
router.put('/wiki/spaces/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (updates.length) { updates.push("updated_at = datetime('now')"); params.push(req.params.id); run(db, `UPDATE wiki_spaces SET ${updates.join(', ')} WHERE id = ?`, params); }
    res.json(queryOne(db, 'SELECT * FROM wiki_spaces WHERE id = ?', [req.params.id]));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/wiki/spaces/:id', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const pages = queryAll(db, 'SELECT id FROM wiki_pages WHERE space_id = ?', [req.params.id]);
    pages.forEach(p => {
      run(db, 'DELETE FROM wiki_page_comments WHERE page_id = ?', [p.id]);
      run(db, 'DELETE FROM wiki_page_versions WHERE page_id = ?', [p.id]);
      run(db, 'DELETE FROM wiki_labels WHERE page_id = ?', [p.id]);
      run(db, 'DELETE FROM wiki_stars WHERE page_id = ?', [p.id]);
      run(db, 'DELETE FROM wiki_page_likes WHERE page_id = ?', [p.id]);
    });
    run(db, 'DELETE FROM wiki_pages WHERE space_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_templates WHERE space_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_stars WHERE space_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_spaces WHERE id = ?', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/wiki/spaces/:spaceId/pages', requireAuth, async (req, res) => {
  try {
    ...
    const maxPos = queryOne(db, 'SELECT COALESCE(MAX(position),0)+1 as p FROM wiki_pages WHERE space_id = ? AND parent_id IS ?', [req.params.spaceId, parent_id || null]);
    run(db, `INSERT INTO wiki_pages (id, space_id, parent_id, title, slug, body, body_format, author_id, last_editor_id, position) VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [id, req.params.spaceId, parent_id || null, title, slug, body || '', body_format || 'html', req.userId, req.userId, maxPos.p]);
    run(db, "INSERT INTO wiki_page_versions (id, page_id, version, title, body, editor_id, change_message) VALUES (?,?,1,?,?,?,'Created')",
      [uuidv4(), id, title, body || '', req.userId]);
    if (labels && Array.isArray(labels)) {
      labels.forEach(l => { try { run(db, "INSERT INTO wiki_labels (id, page_id, label) VALUES (?,?,?)", [uuidv4(), id, l.toLowerCase().trim()]); } catch(e){} });
    }
    const page = queryOne(db, 'SELECT * FROM wiki_pages WHERE id = ?', [id]);
...
router.get('/wiki/pages/:id', requireAuth, async (req, res) => {
  try {
    ...
    // Increment view
    run(db, 'UPDATE wiki_pages SET view_count = view_count + 1 WHERE id = ?', [req.params.id]);
    // Labels
    page.labels = queryAll(db, 'SELECT label FROM wiki_labels WHERE page_id = ?', [req.params.id]).map(l => l.label);
    // Breadcrumb
    const breadcrumb = [];
    let current = page;
    while (current.parent_id) {
      const parent = queryOne(db, 'SELECT id, title, parent_id FROM wiki_pages WHERE id = ?', [current.parent_id]);
      if (!parent) break;
    ...
    // Children
    page.children = queryAll(db, "SELECT id, title, slug, status FROM wiki_pages WHERE parent_id = ? AND status = 'current' ORDER BY position, title", [req.params.id]);
    res.json(page);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/wiki/pages/:id', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const page = queryOne(db, 'SELECT * FROM wiki_pages WHERE id = ?', [req.params.id]);
    if (!page) return res.status(404).json({ error: 'Page not found' });
    ...
    if (title !== undefined || body !== undefined) {
      ...
      run(db, "INSERT INTO wiki_page_versions (id, page_id, version, title, body, editor_id, change_message) VALUES (?,?,?,?,?,?,?)",
        [uuidv4(), req.params.id, newVersion, title || page.title, body || page.body, req.userId, change_message || '']);
    }
    updates.push("updated_at = datetime('now')");
    params.push(req.params.id);
    run(db, `UPDATE wiki_pages SET ${updates.join(', ')} WHERE id = ?`, params);
    const updated = queryOne(db, 'SELECT * FROM wiki_pages WHERE id = ?', [req.params.id]);
    res.json(updated);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/wiki/pages/:id', requireAuth, async (req, res) => {
  try {
    ...
    const page = queryOne(db, 'SELECT parent_id FROM wiki_pages WHERE id = ?', [req.params.id]);
    run(db, 'UPDATE wiki_pages SET parent_id = ? WHERE parent_id = ?', [page?.parent_id || null, req.params.id]);
    run(db, 'DELETE FROM wiki_page_comments WHERE page_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_page_versions WHERE page_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_labels WHERE page_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_stars WHERE page_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_page_likes WHERE page_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_pages WHERE id = ?', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/wiki/pages/:id/comments', requireAuth, async (req, res) => {
  try {
    ...
    const id = uuidv4();
    run(db, "INSERT INTO wiki_page_comments (id, page_id, parent_id, author_id, body) VALUES (?,?,?,?,?)",
      [id, req.params.id, parent_id || null, req.userId, body]);
    const comment = queryOne(db, `SELECT c.*, u.name as author_name, u.avatar_color as author_color FROM wiki_page_comments c LEFT JOIN users u ON c.author_id = u.id WHERE c.id = ?`, [id]);
...
router.delete('/wiki/comments/:id', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    run(db, 'DELETE FROM wiki_page_comments WHERE parent_id = ?', [req.params.id]);
    run(db, 'DELETE FROM wiki_page_comments WHERE id = ?', [req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/wiki/pages/:id/labels', requireAuth, async (req, res) => {
  try {
    ...
    try { run(db, "INSERT INTO wiki_labels (id, page_id, label) VALUES (?,?,?)", [uuidv4(), req.params.id, label.toLowerCase().trim()]); } catch(e){}
    const labels = queryAll(db, 'SELECT label FROM wiki_labels WHERE page_id = ?', [req.params.id]).map(l => l.label);
    res.json(labels);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/wiki/pages/:pageId/labels/:label', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    run(db, 'DELETE FROM wiki_labels WHERE page_id = ? AND label = ?', [req.params.pageId, req.params.label]);
    const labels = queryAll(db, 'SELECT label FROM wiki_labels WHERE page_id = ?', [req.params.pageId]).map(l => l.label);
    res.json(labels);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/wiki/star', requireAuth, async (req, res) => {
  try {
    ...
    const existing = page_id
      ? queryOne(db, 'SELECT id FROM wiki_stars WHERE user_id = ? AND page_id = ?', [req.userId, page_id])
      : queryOne(db, 'SELECT id FROM wiki_stars WHERE user_id = ? AND space_id = ?', [req.userId, space_id]);
    if (existing) {
      run(db, 'DELETE FROM wiki_stars WHERE id = ?', [existing.id]);
      res.json({ starred: false });
    } else {
      run(db, "INSERT INTO wiki_stars (id, user_id, page_id, space_id) VALUES (?,?,?,?)", [uuidv4(), req.userId, page_id || null, space_id || null]);
...
router.post('/wiki/pages/:id/like', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const existing = queryOne(db, 'SELECT id FROM wiki_page_likes WHERE page_id = ? AND user_id = ?', [req.params.id, req.userId]);
    if (existing) {
      run(db, 'DELETE FROM wiki_page_likes WHERE id = ?', [existing.id]);
      res.json({ liked: false });
    } else {
      run(db, "INSERT INTO wiki_page_likes (id, page_id, user_id) VALUES (?,?,?)", [uuidv4(), req.params.id, req.userId]);
      res.json({ liked: true });
    }
    const count = queryOne(db, 'SELECT COUNT(*) as c FROM wiki_page_likes WHERE page_id = ?', [req.params.id]);
...
router.get('/wiki/spaces/:spaceId/tree', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const pages = queryAll(db, "SELECT id, title, parent_id, position, slug FROM wiki_pages WHERE space_id = ? AND status = 'current' ORDER BY position, title", [req.params.spaceId]);
    // Build tree
