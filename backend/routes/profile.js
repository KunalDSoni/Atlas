...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    // Get projects
    const projects = queryAll(db, `SELECT p.id, p.name, p.key, pm.role_in_project FROM project_members pm JOIN projects p ON p.id = pm.project_id WHERE pm.user_id = ?`, [req.userId]);

    // Get stats
    const assigned = queryOne(db, `SELECT COUNT(*) as count FROM issues WHERE assignee_id = ? AND status != 'done'`, [req.userId]);
    const completed = queryOne(db, `SELECT COUNT(*) as count FROM issues WHERE assignee_id = ? AND status = 'done'`, [req.userId]);
    const comments = queryOne(db, `SELECT COUNT(*) as count FROM comments WHERE author_id = ?`, [req.userId]);
...
router.put('/password', requireAuth, async (req, res) => {
  try {
    ...
    const db = await getDb();
    const user = queryOne(db, 'SELECT password FROM users WHERE id = ?', [req.userId]);
    if (user.password !== current_password) return res.status(400).json({ error: 'Current password is incorrect' });

    run(db, 'UPDATE users SET password = ? WHERE id = ?', [new_password, req.userId]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/email', requireAuth, async (req, res) => {
  try {
    ...
    const db = await getDb();
    const existing = queryOne(db, 'SELECT id FROM users WHERE email = ? AND id != ?', [email, req.userId]);
    if (existing) return res.status(400).json({ error: 'Email already taken' });

    run(db, 'UPDATE users SET email = ? WHERE id = ?', [email, req.userId]);
    const user = queryOne(db, 'SELECT id, name, email, role, avatar_color FROM users WHERE id = ?', [req.userId]);
    res.json(user);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
