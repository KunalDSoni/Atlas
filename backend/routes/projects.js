...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const projects = await queryAll(db, `
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const db = await getDb();
    const id = uuidv4();
    await run(db, 'INSERT INTO projects (id, name, key, description, lead_id) VALUES (?, ?, ?, ?, ?)',
      [id, name, key.toUpperCase(), description || '', lead_id || req.userId]);

    // Add lead as project member
    if (lead_id) {
      await run(db, 'INSERT INTO project_members (id, project_id, user_id, role_in_project) VALUES (?, ?, ?, ?)',
        [uuidv4(), id, lead_id, 'lead']);
    }

    const project = await queryOne(db, 'SELECT * FROM projects WHERE id = ?', [id]);
...
router.put('/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (updates.length > 0) {
      ...
      await run(db, `UPDATE projects SET ${updates.join(', ')} WHERE id = ?`, params);
    }

    const updated = await queryOne(db, 'SELECT * FROM projects WHERE id = ?', [req.params.id]);
    res.json(updated);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    ...
    // Cascade delete
    await run(db, 'DELETE FROM comments WHERE issue_id IN (SELECT id FROM issues WHERE project_id = ?)', [pid]);
    await run(db, 'DELETE FROM issues WHERE project_id = ?', [pid]);
    await run(db, 'DELETE FROM sprints WHERE project_id = ?', [pid]);
    await run(db, 'DELETE FROM project_members WHERE project_id = ?', [pid]);
    const proj = await queryOne(db, 'SELECT name, key FROM projects WHERE id = ?', [pid]);
    await run(db, 'DELETE FROM projects WHERE id = ?', [pid]);
...
router.get('/:id/stats', requireAuth, async (req, res) => {
  try {
    const db = await getDb();
    const pid = req.params.id;
    const total = (await queryOne(db, 'SELECT COUNT(*) as count FROM issues WHERE project_id = ?', [pid])).count;
    const byStatus = await queryAll(db, 'SELECT status, COUNT(*) as count FROM issues WHERE project_id = ? GROUP BY status', [pid]);
    const byType = await queryAll(db, 'SELECT type, COUNT(*) as count FROM issues WHERE project_id = ? GROUP BY type', [pid]);
    const byAssignee = await queryAll(db, `
...
router.post('/:id/members', requireAuth, async (req, res) => {
  try {
    ...
    const db = await getDb();
    const existing = await queryOne(db, 'SELECT id FROM project_members WHERE project_id = ? AND user_id = ?', [req.params.id, user_id]);
...
router.put('/:id/members/:userId', requireAuth, async (req, res) => {
  try {
    ...
    await run(db, 'UPDATE project_members SET role_in_project = ? WHERE project_id = ? AND user_id = ?',
